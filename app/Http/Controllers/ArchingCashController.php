...
class ArchingCashController extends Controller
{
    ...
    public function get()
    {
        //$validation = DB::table('model_has_roles')->select("name")->where([["model_id", "=", Auth::user()->id], ["role_id", "!=", 3]])->get();
    ...
    public function get_summary(Request $request)
    {
        ...
        // Sales
        $sales              = DB::select("SELECT SUM(monto) as monto, idpago, pay_modes.descripcion as tipo_pago, COUNT(idpago) as cantidad
        ...
        $html_quantity      ='';
        $sales_empty        = null;
        $html_download      = '<button type="button" class="btn btn-label-secondary"
    ...
    {
        $data["pagos"] = DB::select("SELECT SUM(monto) as monto, COUNT(idpago) as cant, pay_modes.descripcion as tipo_pago
        FROM detail_payments
        INNER JOIN pay_modes ON detail_payments.idpago = pay_modes.id
        WHERE idcaja = $id 
