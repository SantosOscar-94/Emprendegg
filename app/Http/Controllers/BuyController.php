...
class BuyController extends Controller
{
    ...
    public function save(Request $request)
    {
        ...
        foreach ($cart['products'] as $product) {
            ...
            $stock__actual              = StockProduct::where('idproducto', $product["id"])->where('idalmacen', $idalmacen)->first();
            StockProduct::where('idproducto', $product["id"])->where('idalmacen', $idalmacen)->update([
                'cantidad'  => $stock__actual->cantidad + $product["cantidad"]
            ]);
    ...
    public function delete(Request $request)
    {
        ...
        foreach ($detail_buy as $item) {
            ...
            $stock__actual  = StockProduct::where('idproducto', $id_product)->where('idalmacen', $buy->idalmacen)->first();
            
            if (((int) $stock__actual->cantidad - $cantidad) <= 0) {
                StockProduct::where('idproducto', $id_product)->where('idalmacen', $buy->idalmacen)->update([
                    'cantidad'  => 0
                ]);
            } else {
                StockProduct::where('idproducto', $id_product)->where('idalmacen', $buy->idalmacen)->update([
