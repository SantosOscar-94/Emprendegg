...
class PosController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['cuentas']            = Cuentas::orderBy('id', 'desc')->first();
    ...
    public function view_products(Request $request)
    {
        ...
        $html_products  = '';
        $products       = DB::select("CALL get_lista_productos()");
        $userWareHouse = Warehouse::findOrFail(Auth::user()['idalmacen']);
        // $products = DB::table('stock_products')
        // ->join('products', 'stock_products.idproducto', '=', 'products.id')
        // ->join('warehouses', 'stock_products.idalmacen', '=', 'warehouses.user_id')
        ...
        if (!empty($products)) {
            foreach ($products as $product) {
                ...
                if ($product->descrip == $userWareHouse->descripcion) { 
                    $html_products .= '<div class="col-12 col-sm-6 col-md-6 col-lg-4 col-xl-3 mb-2 item" style="cursor: pointer;" >
                                            <div id="' . $product->id . '" class="card h-100  btn-add-product-cart" data-id="' . $product->id . '" data-cantidad="1" data-cant="'.$product->cantidad.'" data-precio="' . $product->precio_venta . '" data-precio2="'.  $product->precio_venta_por_mayor .'" data-precio3="'.  $product->precio_venta_distribuidor .'">
                                                <div class="card-body" style="border-radius: 5px ">';
                    if (!empty($product->codigo_interno))
                        $html_products .= '<small class="fw-bold">STOCK= '. $product->cantidad . '  </small>';
                        $html_products .= '<small class="fw-bold">  '. $product->descrip . '  </small>';
                    $html_products .= '<h6 class="mb-2 pb-1">' . $product->descripcion . '</h6>
                                                    ...
                                                    <div class="row mb-3 g-3">
                                                        <div class="col-6">
                                                            <div class="d-flex">
                                                                <div>
                                                                    <h6 class="mb-0 fw-bold text-nowrap text-primary" id="itemPrice'. $product->id .'" data-precio="' . $product->precio_venta . '" data-precio2="'.  $product->precio_venta_por_mayor .'" data-precio3="'.  $product->precio_venta_distribuidor .'">
                                                                        S/ ' . number_format($product->precio_venta, 2, ".", "") . '
                                                                    </h6><br>';
                    // if ($product->cantidad == 0) {
                    //     //$html_products .= '<a href="http://multi-sucursal.test/transfer-orders" class="btn btn-primary">Solicitar traslado</a>';
                    //     //$html_products .= '<script>window.cant = '+ $product->cantidad +'</script>';
                    // }
                    $html_products .='                           </div>
                                                            </div>
                                                        </div>
    ...
    public function search_view_product(Request $request)
    {
        ...
        $products = Cache::rememberForever('search-products', function () use ($value) {
            return StockProduct::select("products.*", "stock_products.cantidad as cantidad", "warehouses.descripcion as descrip")
                ->join('products', 'stock_products.idproducto', '=', 'products.id')
                ->join('warehouses', 'stock_products.idalmacen', '=', 'warehouses.id')
                ->where('products.descripcion', 'like', "%$value%")
                ->orWhere('products.marca', 'like', "%$value%")
                ->orWhere('products.presentacion', 'like', "%$value%")
    ...
    public function save(Request $request)
    {
        ...
        $cuenta                 = ($quantity_paying_3 != 0) ? $request->input('cuenta') : '0';
        $referencia2            = $request->input('referencia2');
        $cuenta2                = ($quantity_paying_2 != 0) ? $request->input('cuenta2') : '0';
