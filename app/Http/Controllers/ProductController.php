...
class ProductController extends Controller
{
    ...
    public function download(Request $request)
    {
        ...
        foreach ($data['productos'] as $index => $producto) {
            ...
            {
                $stocks .=  StockProduct::select('cantidad')->where('idproducto', $producto["id"])->where('idalmacen', $store["id"])->first()->cantidad;
                $c < $store->count () ? $stocks .= ';' : '';
                $c++;
            }
            $data['productos'][$index]['stocks'] = $stocks;
        }
